...
export function createGround(scene, colorHex) {
    ...
    mesh.name = "ground_terrain"; // Tag for Minimap
    scene.add(mesh);
}

// UPDATED: Added 'hasCollision' parameter
export function createRiver(scene, x, z, width, length, hasCollision = true) {
    ...
    mesh.receiveShadow = true;
    mesh.name = "water_terrain"; // Tag for Minimap
    scene.add(mesh);
...
export function createPath(scene, x, z, width, length, rotY=0) {
    ...
    mesh.position.set(x, 0.06, z); // Slightly higher than water/ground
    mesh.receiveShadow = true;
    mesh.name = "path_terrain"; // Tag for Minimap
    scene.add(mesh);
}

export function createBridge(scene, x, z, width = 8, length = 4) {
    // A physical bridge mesh (Arch style)
    const bridge = new THREE.Mesh(new THREE.BoxGeometry(width, 0.5, length), new THREE.MeshStandardMaterial({ color: 0x5c4033 }));
